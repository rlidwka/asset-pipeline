...
(function() {
  var Callback, Monad, Path, Wrap, async, crypto, escape_chars, fs, get_digest, monads, util;
  ...
  get_digest = function get_digest(file) {
    var md5, res;
  ...
  module.exports.prepare = function prepare(gopts) {
    ...
    get_file = function get_file(file, cb) {
      ...
      return gopts.pipeline.compile_file(file, function(err) {
        if (err) return cb(err);
        return fs.readFile(gopts.pipeline.req_to_cache(file), function(err) {
    ...
    Inlines.asset_digest = Wrap(function(file, options) {
      ...
      callback = new Callback();
      file = Path.resolve(Path.dirname(filename), file);
      get_file(file, function(err, res) {
        var digest;
    ...
    Inlines.asset_md5 = Wrap(function(file, options) {
      ...
      get_file(file, function(err, res) {
        ...
        md5 = crypto.createHash('md5');
        md5.update(res);
        return callback.set([null, md5.digest('hex')]);
      });
      return callback.func();
    });
    ...
    Inlines.asset_uri = Wrap(function(file, options) {
      ...
      file = Path.resolve(Path.dirname(filename), file);
      get_file(file, function(err, res) {
        var base, digest, ext, result, _ref;
        if (err) return callback.set(arguments);
        digest = get_digest(res);
        ...
        return util.link_file(gopts.pipeline.req_to_cache(file), gopts.pipeline.req_to_cache(result), function(err, res) {
