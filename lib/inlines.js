...
(function() {
  var Cache, Callback, END_SEQ, FuncID, Monad, Path, START_SEQ, Wrap, async, crypto, escape_chars, fs, gen_code, inlines_call, monads, resolve, util;
  ...
  resolve = function resolve() {
    return Path.resolve.apply(Path, arguments).replace(/^[A-Z]:\\/, '');
  };

  gen_code = function gen_code(length) {
    var chars, code, i, _i;
    code = '';
  ...
  inlines_call = function inlines_call(orig, result, pos, cb) {
  ...
  module.exports.call = function call(code, cb) {
  ...
  module.exports.prepare = function prepare(gopts) {
    ...
    get_digest = function get_digest(file, cb) {
      var _ref;
      if ((_ref = pipeline._digest_cache) == null) {
        pipeline._digest_cache = new Cache({
          max: 10000,
          maxAge: 2 * 60 * 60 * 1000,
          load: function load(file, cb) {
